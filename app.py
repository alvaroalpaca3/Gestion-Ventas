...
def conectar_google():
    try:
        ...
        st.error("⚠️ Error de enlace: Revisa st.secrets.")
        return None
...
def cargar_datos():
    ...
    try:
        ...
        df_est['DNI'] = df_est['DNI'].fillna('').astype(str).str.strip()
...
# --- 3. LOGIN (Validación DNI 71914998 y otros) ---
...
dni_input = st.sidebar.text_input("DNI / CE VENDEDOR", max_chars=9).strip()
# Buscamos ignorando ceros a la izquierda y decimales .0
dni_busqueda = "".join(filter(str.isdigit, dni_input)).lstrip('0')

if len(dni_busqueda) >= 6:
    if not df_maestro.empty:
        # Limpieza profunda en el Maestro para match perfecto
        df_maestro['DNI_MATCH'] = (
            df_maestro['DNI']
            .astype(str)
        ...
        vendedor = df_maestro[df_maestro['DNI_MATCH'] == dni_busqueda]
        
        if not vendedor.empty:
            st.session_state.nom_v = vendedor.iloc[0]['NOMBRE VENDEDOR']
            st.session_state.zon_v = vendedor.iloc[0]['ZONAL']
            st.session_state.sup_v = vendedor.iloc[0]['SUPERVISOR']
...
with tab1:
    if nom_v == "N/A":
        st.info("👈 Ingresa tu DNI en la barra lateral.")
    else:
        ...
        with st.form(key=f"f_{st.session_state.form_key}"):
            ...
            if detalle == "NO-VENTA":
                m_nv = st.selectbox("MOTIVO *", ["COMPETENCIA", "MALA EXPERIENCIA", "CARGO ALTO", "SIN COBERTURA", "YA TIENE SERVICIO"], index=None, placeholder="Elija un motivo...")
                st.info("💡 Solo elija el motivo. Sus datos se cargan del login.")
            
            elif detalle == "REFERIDO":
                n_ref = st.text_input("Nombre Referido *").upper()
                c_ref = st.text_input("Celular Referido (9) *", max_chars=9)
                ...
                with ca:
                    ...
                    t_op = st.selectbox("Operación *", ["SELECCIONA", "CAPTACIÓN", "MIGRACIÓN", "COMPLETA TV", "COMPLETA BA"])
                    prod = st.selectbox("Producto *", ["SELECCIONA", "NAKED", "DUO INT + TV", "DUO BA", "TRIO"])
                    pil = st.radio("Piloto?", ["NO", "SI"], horizontal=True)
                with cb:
                    dir_ins = st.text_input("Dirección *").upper()
                    c1 = st.text_input("Celular (9) *", max_chars=9)
                    n_ped = st.text_input("N° Orden (10) *", max_chars=10)
                    mail = st.text_input("Email *")
                    c_fe = st.text_input("Código FE (13) *", max_chars=13)

            if st.form_submit_button("💾 GUARDAR GESTIÓN", use_container_width=True):
                err = False
                if detalle == "SELECCIONA": st.error("Elija gestión"); err = True
                elif detalle == "NO-VENTA" and not m_nv: st.error("Elija motivo"); err = True
                elif detalle == "REFERIDO" and (not n_ref or len(c_ref) != 9): st.error("Datos incompletos"); err = True
                elif detalle in ["VENTA FIJA", "CLIENTE AGENDADO"]:
                    if any(x == "SELECCIONA" or not str(x).strip() for x in [n_cl, d_cl, dir_ins, c1, n_ped, c_fe, t_op, prod]):
                        st.error("❌ Complete todos los campos (*)"); err = True
                    elif len(d_cl) != 8: st.error("❌ DNI debe tener 8 dígitos"); err = True
                    elif len(c1) != 9: st.error("❌ Celular debe tener 9 dígitos"); err = True
                    elif len(n_ped) != 10: st.error("❌ N° Pedido debe tener 10 dígitos"); err = True
                    elif len(c_fe) != 13: st.error("❌ Código FE debe tener 13 dígitos"); err = True

                if not err:
                    try:
                        tz = pytz.timezone('America/Lima')
                        ahora = datetime.now(tz)
                        # Guardamos con apóstrofe para conservar ceros
                        fila = [
